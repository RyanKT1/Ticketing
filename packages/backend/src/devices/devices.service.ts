...
export class DevicesService {
  ...
  async findAll(): Promise<SuccessResponse | ErrorResponse> {
    try {
      ...
      return makeErrorResponse(ErrorTypes[ErrorCode.DATABASE_ERROR], 'Failed to retrieve devices');
    }
  }
  ...
  async findOne(id: string): Promise<SuccessResponse | ErrorResponse> {
    try {
      ...
      return makeErrorResponse(ErrorTypes[ErrorCode.DATABASE_ERROR], 'Failed to retrieve device');
    }
  }
  ...
  async create(createDeviceDto: CreateDeviceDto): Promise<SuccessResponse | ErrorResponse> {
    try {
      ...
      await this.devicesRepository.upsertOneDevice(
        Device.createDeviceInstanceFromDeviceDto(newDevice),
      );
      return makeSuccessResponse(null, HttpStatus.CREATED);
    } catch (error) {
      this.logger.error(`Failed to create device: ${error.message}`);
      return makeErrorResponse(ErrorTypes[ErrorCode.DATABASE_ERROR], 'Failed to create device');
    }
  }
  ...
  async update(
    ...
    try {
      ...
      return makeErrorResponse(ErrorTypes[ErrorCode.DATABASE_ERROR], 'Failed to update device');
    }
  }
  ...
  async delete(id: string): Promise<SuccessResponse | ErrorResponse> {
    try {
      ...
      return makeErrorResponse(ErrorTypes[ErrorCode.DATABASE_ERROR], 'Failed to delete device');
    }
  }
}
