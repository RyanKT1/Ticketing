...
function TicketManagementPage() {
  const auth = useAuth();
  const currentUser = auth.user?.profile?.['cognito:username'];
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      try {
        ...
        setTickets(safeTicketList);
        setTotalPages(Math.ceil(safeTicketList.length / ticketsPerPage));
      } catch (error) {
  ...
  const handleResolveTicket = async (ticketId, markAsResolved = true) => {
    try {
      ...
      const updatedTicket = { ...ticketToUpdate, resolved: markAsResolved };

      await updateTicket(ticketId, updatedTicket, auth);

      setTickets(prevTickets =>
        prevTickets.map(ticket =>
          ticket.id === ticketId ? { ...ticket, resolved: markAsResolved } : ticket
        )
      );
      ...
      setModalMessage(
        error.message ||
          `Failed to ${markAsResolved ? 'resolve' : 'reopen'} ticket. Please try again.`
      );
      setShowErrorModal(true);
    }
  };
  ...
  const getFilteredTickets = () => {
    ...
    return tickets.filter(ticket => (filter === 'resolved' ? ticket.resolved : !ticket.resolved));
  };
